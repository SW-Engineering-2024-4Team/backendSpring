...
public class PlantAndHarvestTest {
    ...
    @Test
    public void testPlantAndHarvest() {
        ...
        // 수확 단계 실행
        gameController.farmPhase();
        printPlayerBoardWithFields("Player board after harvest phase:");
        printPlayerResources("Player resources after harvest phase:");

        // 검증
        Tile[][] tiles = player.getPlayerBoard().getTiles();
        boolean grainHarvested = false;
        int grainCount = player.getResource("grain");

        for (int i = 0; i < tiles.length; i++) {
            for (int j = 0; j < tiles[0].length; j++) {
                if (tiles[i][j] instanceof FieldTile) {
                    FieldTile field = (FieldTile) tiles[i][j];
                    if (field.getCropType().equals("grain") && field.getCrops() == 2) {
                        grainHarvested = true;
                    }
                }
            }
        }
        assertTrue(grainHarvested, "Grain should be harvested from the field.");
        assertEquals(5, grainCount, "Player should have 5 grains after harvest (initial 5 - 1 plant + 1 harvested).");
    }
}
